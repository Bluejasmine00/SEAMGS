...
class _DistributedOptimizer(torch.optim.Optimizer):
    ...
    def _generate_groups_with_lags(self):
        print("--------------------lags-sgd---------------------")
        P = size()
        alpha = 1.6e-3
        beta = 1.0e-8
    ...
    def _generate_groups_snamgs(self): # 新定义的分组策略：稀疏噪声感知的tensorfusion
        print("--------------------snamgs-sgd---------------------")
        P = size()
        # alpha = 9.618801111215886e-08 
        # beta = 3.89407453e-13
        # alpha = 6.1e-05 # nccl-tests测试得到通信模型的参数
        # beta = 1.8e-05
        alpha = 1.6e-3
        beta = 1.0e-8
        ...
        for l in range(1, L-1)[::-1]: # L-1到1
            ...
            if current_taob < taoc[l]+tc[l]:
                ...
                t_saved = alpha
                if t_wait < t_saved and (rho[l]-rho[l-1])*(w[l]-w[l-1])<=0:
                    __merge(tc, p, l, g, rho, w)
                    taoc = __calculate_comm_start(tc, tb, taob, L)
                    merged=True
            if not merged:
                idx += 1
    ...
    def _generate_merged_parameters(self):
        ...
        if ADAPTIVE_MERGE and self._layerwise_times is not None:
            if self._density < 1: # MGS 
                groups, key_groupidx_maps = self._generate_groups_snamgs()
                # groups, key_groupidx_maps = self._generate_groups_with_lags()
            else:
